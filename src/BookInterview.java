...
@WebServlet("/BookInterview")
public class BookInterview extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public BookInterview() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		boolean book = false;
		if("book".equals(action)) book = true;
		System.out.println(day + " " + time + " " + book);
		try {
			if(book) System.out.println(DumInterface.bookInterview(sessionNumber, day, time));
			else System.out.print(DumInterface.unbookInterview(sessionNumber));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
