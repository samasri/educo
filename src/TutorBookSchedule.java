...
@WebServlet("/TutorBookSchedule")
public class TutorBookSchedule extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public TutorBookSchedule() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getSession().setAttribute("tutorEmail", TutorEmail);
		String[][] scheduleContent = null;
		try {
			scheduleContent = DumInterface.getSchedule(sessionNumber, TutorEmail);
		...
		for(int i = 0; i < schedule.length; i++) {
			...
			for(int j = 0; j < 6; j++) {
				if("-1".equals(current[j]))
					writer.write("\t\t\t\t<td id='" + j + "'class='unavailable'> </td>\n");
				else if("".equals(current[j])) writer.write("\t\t\t\t<td id='" + j + "'class='available'>" + current[j] + "</td>\n");
	...
	public void writePopup(PrintWriter writer) {
		//popup
